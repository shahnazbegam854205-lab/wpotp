...
// 5. Get Reseller Prices (Public - For referred users)
app.get('/api/reseller/prices', async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: `Prices include ${commissionPercent}% commission`
    });
...
app.get('/api/getNumber', async (req, res) => {
  try {
    ...
    // Check ref from query or user's referredBy
    const referralCode = ref || user.referredBy;
    ...
    if (referralCode) {
      ...
      if (reseller) {
        ...
        // Save referral in user data if not already
        if (!user.referredBy) {
