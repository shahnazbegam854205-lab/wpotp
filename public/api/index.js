...
<head>
  ...
  <title>Happy OTP Panel - User Dashboard</title>
...
<div id="newKeyModal" class="modal-backdrop">
  ...
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <header class="header-container p-4 rounded-xl">
      <div class="flex justify-between items-center">
        ...
        <div class="logo-container flex items-center gap-3">
          ...
          <div>
            <h1 class="premium-title text-2xl sm:text-3xl">HAPPY OTP PANEL</h1>
            <p class="text-gray-400 text-sm">Virtual Numbers for WhatsApp Verification</p>
          </div>
        </div>
          ...
          <div id="selectedService" class="p-4 bg-bg-lighter rounded-lg mb-4 hidden">
            <div class="flex justify-between items-center">
              <div class="flex items-center gap-3">
                <span id="selectedFlag" class="text-2xl">🇵🇭</span>
                <div>
              ...
              <div class="text-2xl font-bold" style="color: #00ff9d;" id="selectedPrice">₹52</div>
            </div>
          </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="stat-card">
        <div class="text-2xl font-bold text-primary-green" id="totalSpent">₹0</div>
  ...
  <script>
    ...
    [welcomeAudio, createAudio, loginAudio, numberPurchaseAudio, cancelAudio, 
     refreshAudio, supportAudio, apiKeyAudio, balanceAudio, apiDocsAudio, logoutAudio]
    ...
    async function getNumberWithSound() {
      playRefreshSound(); // Play refresh sound for buy action
      await getNumber();
    }
    ...
    async function cancelNumberWithSound() {
      if (confirm('Are you sure you want to cancel this number?')) {
        playRefreshSound(); // Initial sound
        await cancelNumber();
      }
    }
          ...
          html += `
            <div class="country-option mb-2 rounded p-3" data-key="${service.key}">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                  <div class="font-bold" style="color: #00ff9d;">₹${service.price}</div>
    ...
    function selectService(serviceKey) {
      ...
      selectedPrice.textContent = `₹${service.price}`;
    ...
    async function getNumber() {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/getNumber?api_key=${userApiKey}&country=${encodeURIComponent(selectedService)}`
        );
        ...
        if (data.success) {
          ...
          showStatus(`Virtual number purchased successfully! ₹${selectedServicePrice} deducted.`, 'success');
        } else {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.refundAmount > 0) {
            showStatus(`Number cancelled successfully. ₹${data.refundAmount} refunded.`, 'success');
          } else {
      ...
      if (user) {
        ...
        try {
          ...
          if (userData) {
            ...
            profileBalance.textContent = `₹${userData.wallet || 0}`;
            balanceDisplay.textContent = `₹${userData.wallet || 0}`;
          }
        } catch (error) {
